...
dae::FirstLevel::FirstLevel(const std::string& name) :Scene(name)
, m_GridSizeWidth(14)
, m_GridSizeHeight(19)
{
...
void dae::FirstLevel::Initialize()
{
...
void dae::FirstLevel::Update()
{
	...
	if (!m_Fygar && ! m_Pooka)
	{
		...
		SceneManager::GetInstance().DeleteActiveScene();
		SceneManager::GetInstance().CreateScene("SecondLevel", SceneManager::SecondLevel);
	}
}

void dae::FirstLevel::Render() const
{
	Scene::Render();
}

void dae::FirstLevel::Reset()
{
	Scene::Reset();
	m_Rocks.clear();
