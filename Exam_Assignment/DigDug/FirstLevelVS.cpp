...
dae::FirstLevelVS::FirstLevelVS(const std::string& name) :Scene(name)
, m_GridSizeWidth(14)
, m_GridSizeHeight(19)
{
...
void dae::FirstLevelVS::Initialize()
{
	...
	m_Fygar->AddComponent(std::make_shared<Enemies::FygarComponent>(grids, gun,Controllers::PLAYER02));
...
void dae::FirstLevelVS::Update()
{
	...
	if (!m_Fygar && !m_Pooka)
	{
		...
		SceneManager::GetInstance().DeleteActiveScene();
		SceneManager::GetInstance().CreateScene("SecondLevel", SceneManager::SecondLevel2v2);
	}
}

void dae::FirstLevelVS::Render() const
{
	Scene::Render();
}

void dae::FirstLevelVS::Reset()
{
	Scene::Reset();
	m_Rocks.clear();
